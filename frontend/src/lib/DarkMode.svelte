<script>
    ...
    let active = $derived(themeOps.get(theme) ?? 2);
    ...
    onMount(() => {
        ...
        if (themeOps.get(theme) === null || themeOps.get(theme) === undefined) {
            theme = 'auto';
        }

        let callback = (/** @type {MediaQueryListEvent} */ e) => {
        ...
        active = themeOps.get(theme) ?? 2;
    });

    const changeTheme = (/** @type {number} */ i) => {
        active = i;
        switch (i) {
            case 0:
                theme = 'light';
...
<ButtonGroup class="inline-flex rounded-lg shadow-sm my-1">
